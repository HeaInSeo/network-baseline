...
- `/v2/` registry endpoint 접근 실패를 image pull 실패와 분리한다.
- Harbor 정본 registry 실패와 GHCR mirror 실패를 별도 evidence로 남긴다.
...
`FETCH_URL`이 없으면 결과는 `skipped`로 기록된다. URL이 있으면 Kubernetes
Job 안에서 HTTP fetch를 수행하고 `sizeBytes`, `fetchSeconds`, `sha256`,
`digestVerified`, `cleanupObserved`, `failureLayer`를 남긴다. 이 baseline은
object storage나 특정 transfer backend를 전제로 하지 않는다.
